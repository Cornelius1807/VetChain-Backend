...
app.put("/me", auth(), async (req: any, res) => {
  const cuenta = await prisma.cuenta.findUnique({ where: { id: req.user.id } });
  if (!cuenta) return res.status(404).json({ error: "Cuenta no encontrada" });
...
// Admin endpoints
app.get("/admin/pending-vets", auth(), async (req: any, res) => {
...
app.post("/admin/vets/:id/approve", auth(), async (req: any, res) => {
  const me = await prisma.cuenta.findUnique({ where: { id: req.user.id } });
  if (me?.rol !== "admin") return res.status(403).json({ error: "Solo admin" });
...
app.post("/admin/vets/:id/reject", auth(), async (req: any, res) => {
  const me = await prisma.cuenta.findUnique({ where: { id: req.user.id } });
  if (me?.rol !== "admin") return res.status(403).json({ error: "Solo admin" });
  const vet = await prisma.veterinario.findUnique({ where: { id: req.params.id } });
  if (!vet) return res.status(404).json({ error: "No encontrado" });
...
app.post("/centros", auth(), async (req: any, res) => {
  const me = await prisma.cuenta.findUnique({ where: { id: req.user.id } });
  if (me?.rol !== "admin" && me?.rol !== "veterinario") return res.status(403).json({ error: "No autorizado" });
  ...
  const centro = await prisma.centroVeterinario.create({
    data: { nombre, direccion, telefono, email, rangoAtencionInicio, rangoAtencionFin, rangoConsultaInicio, rangoConsultaFin,
      consultorios: { create: (consultorios || []).map((n: any) => ({ nombre: n.nombre || String(n) })) },
    },
  });
...
app.get("/citas/vet", auth(), async (req: any, res) => {
  const me = await prisma.cuenta.findUnique({ where: { id: req.user.id } });
  if (me?.rol !== "veterinario") return res.status(403).json({ error: "Solo veterinarios" });
...
app.get("/historial/:mascotaId", auth(), async (req: any, res) => {
  const me = await prisma.cuenta.findUnique({ where: { id: req.user.id } });
...
// Attend (vet)
app.patch("/citas/:id/atender", auth(), async (req: any, res) => {
  const me = await prisma.cuenta.findUnique({ where: { id: req.user.id } });
  if (me?.rol !== "veterinario") return res.status(403).json({ error: "Solo veterinarios" });
